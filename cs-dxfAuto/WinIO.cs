...
namespace cs_dxfAuto
{
    ...
    public class WinIo
    {
        ...
        public static bool IsLoadDll = false;

        public static void Initialize()
        {
            if (!IsLoadDll == false) return;
            if (InitializeWinIo())
            {
        ...
        /// KeyDown(Key) 模拟普通Key键按下
        public static void KeyDown(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
        ...
        /// KeyUp(Key)模拟普通Key键弹起
        public static void KeyUp(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// KeyDownEx(Key)模拟扩展Key键按下
        public static void KeyDownEx(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
            ...
            KBCWait4IBE();//等待键盘缓冲区为空 
            SetPortVal(KBC_KEY_DATA, (IntPtr)0xE0, 1);//写入扩展键标志信息 
            KBCWait4IBE();//等待键盘缓冲区为空 
            SetPortVal(KBC_KEY_CMD, (IntPtr)0xD2, 1);//发送键盘写入命令 
            KBCWait4IBE();//等待键盘缓冲区为空 
        ...
        /// KeyUpEx(Key)模拟扩展Key键弹起
        public static void KeyUpEx(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// KeyPress(Key)模拟普通Key键按下并弹起一次。其中按下和弹起的默认时间间隔是200毫秒
        public static void KeyPress(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// KeyPress(Key)模拟普通Key键按下并弹起一次。其中按下和弹起的默认时间间隔是第二个参数
        public static void KeyPress(VKKey vKeyCoad, int ms)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// KeyPressEx(Key)模拟扩展按键Key按下并弹起一次。其中按下和弹起的默认时间间隔是200毫秒
        public static void KeyPressEx(VKKey vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// KeyPressEx(Key,Int32)模拟扩展按键Key按下并弹起一次。其中按下和弹起的时间间隔是第二个参数
        public static void KeyPressEx(VKKey vKeyCoad, int ms)
        {
            if (!IsLoadDll == false) return;
            if (!IsInitialize) return;
        ...
        /// Simulate mouse down  
        public static void MouseDown(int vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            int btScancode = 0;
        ...
        /// Simulate mouse up  
        public static void MouseUp(int vKeyCoad)
        {
            if (!IsLoadDll == false) return;
            int btScancode = 0;
            btScancode = MapVirtualKey((byte)vKeyCoad, 0);
